...
class Tab:
    def __init__(self, notebook):
        ...
        self.combobox1 = ttk.Combobox(self.combo_frame, textvariable=self.combo1_var, values=[])
        self.combobox1.grid(row=0, column=0, padx=20, pady=10)

        arrow = Label(self.combo_frame, text='↹')
        arrow.grid(row=0, column=1)

        self.combobox2 = ttk.Combobox(self.combo_frame,textvariable=self.combo2_var, values=[])
    ...
    def place_components(self):
        ...
        self.combo_frame.grid(row=0, column=1, columnspan=1, sticky="nsew")
        self.tree.grid(row=1, column=1, rowspan=2, columnspan=1, sticky="nsew")
    ...
    def run_process(self):
        print(self.combo1_var.get())
        print(self.combo2_var.get())
...
class GraphState(TabState):
    def set_content(self):
        self.tab.traffic.init_graph_figure(resolution=100, node_size=0.5, font_size=5, width=0.5, with_labels=True)
